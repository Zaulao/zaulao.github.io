...
A clear improvement point in this compose is the use of the [separate containers approach](https://github.com/nginx-proxy/nginx-proxy#separate-containers) so that docker.sock is mounted in a different container than the externally accessible one, even if docker.sock is mounted with read-only permissions. I will do this soon!
...
For this page to be the content delivered by default by the reverse proxy, it is necessary that I add the environment variable "DEFAULT_HOST: default.home.lan" in the reverse proxy docker-compose, as indicated in the [documentation](https://github.com/nginx-proxy/nginx-proxy#default_root).
